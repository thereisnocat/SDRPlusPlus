...
| 6 | More radios ✅ | Implement `captureConfigHandler`/`applyConfigHandler` (+ `moduleType`) for source modules beyond RSR200, one at a time, to prove the mechanism is genuinely per-module-additive, not RSR200-specific. **Priority order, per the user (2026-08-16), by which radios are actually used, not by implementation convenience:** 1) **FobosSDR** ✅ and 2) **SDRplay** ✅ (both already touched for phasing support — see `PHASING_PLAN.md` — so their config surface is already familiar; the user calls both "crap radios" they don't use much, but ahead of RTL-SDR regardless) — both done 2026-08-16, full rebuild + bundle + smoke test, commits `e88d64f5`/`04e9d309`. SDRplay's capture/apply also gate every per-device field by which RSP model is currently selected (`openDev.hwVer`), mirroring `selectDev()`'s own per-model branching, since an RSPduo's `dualTuner`/`duoFs`/`duoDecim` fields mean nothing on an RSP2/RSPdx. Along the way, found (but deliberately did not fix, out of scope here) a pre-existing dead-key bug in `RSPduoMenu`: its FM/MW notch checkbox writes config key `"fmmwnotch"` (lowercase n) but `selectDev()` only ever reads `"fmmwNotch"` (capital N) back, so that one setting was never actually persisting across restarts. 3) **Airspy HF+** ✅ and 4) **Perseus** ✅ — both done 2026-08-16, commits `5e4b349d`/`6925b4c1`. Airspy HF+ has the simplest surface yet (no per-model branching: sample rate, AGC mode, HF LNA, attenuation). Perseus doesn't build on macOS by default (`OPT_BUILD_PERSEUS_SOURCE` defaults `OFF` here — no `libperseus-sdr` on this Mac normally) — verified anyway by building `libperseus-sdr` from source locally (`github.com/Microtelecom/libperseus-sdr`, matching the macOS CI job's own recipe) and compiling `perseus_source` against the real header with the option temporarily enabled, then reverting the local build config back to the project default afterward. 5) **RTL-SDR** ✅ — done 2026-08-16, commit `4a3cb83e`, deliberately last: the module actually used for section 2.2's original survey and section 2.1's own example. The user owns one, calls it "completely useless," and didn't want it prioritized just because it's common/convenient to implement against. Replicated a documented librtlsdr quirk (direct-sampling toggle needs AGC/gain re-pushed) that the existing `menuHandler` combo already works around. **All five radios in the user's priority order now have the mechanism, not just RSR200 — this phase is complete.** |
