...
| **6** | Fobos adapter + validation | **Yes** | **Done** — `PORT_HF_DUAL` ("HF1 + HF2 (Phasing)"), a second DDC and `registerChannels()`; the single-port modes are untouched. All three deferred bench checks came back: the `.re`→HF1 mapping is **correct** (35.1 dB apart with an antenna on HF1 only, cross-correlation 0.005), so the one-line fix budgeted for it was not needed; two DDCs cost ~10% each rather than double, with 7.5x realtime headroom at the worst display rate; and the inter-channel phase is **reproducible across a stop/start** — 0.07° spread over 8 restarts, coherence 1.0000, drift under 0.5° within a run — so `phaseCoherent = true` is measured rather than argued, unlike the RSPduo's 233°. Also fixed a driver quirk found on the way: the radio needs ~0.5 s after streaming before it will reopen, so stop-then-start used to fail. On-air nulls still to do. |
